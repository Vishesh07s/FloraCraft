  ...
  return (
    <div style={{ animation: 'toastSlideIn 0.3s ease' }}>
      ...
      {loading ? <Loader/> : error ? <ErrorState message={error}/> : (
        <>
          <PlantGrid items={data.items} onSelect={(p) => navigate(`/product/${p._id}`)}/>
          
          <div className="row" style={{marginTop:'1.5rem', justifyContent:'space-between', alignItems: 'center'}}>
            <div className="small" style={{color: 'var(--text-muted)'}}>{data.total} results</div>
